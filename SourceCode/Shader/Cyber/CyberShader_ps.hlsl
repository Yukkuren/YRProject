...
float sun(vec2 uv, float battery)
{
    ...
    return 0.0;//clamp(val * cut, 0.0, 1.0) + bloom * 0.6;
}
...
float grid(vec2 uv, float battery)
{
    ...
    //uv += vec2(spot_brightness * 4.0 * (battery + 0.05), 0.0);
    uv = abs(fract(uv) - 0.5);
    vec2 lines = smoothstep(size, (0.0), uv);
    lines += smoothstep(size * 5.0, (0.0), uv) * 0.4 * battery;
...
float sdTrapezoid(in vec2 p, in float r1, float r2, float he)
{
    ...
    return 0.0;//s*sqrt( min(dot2(ca),dot2(cb)) );
}
